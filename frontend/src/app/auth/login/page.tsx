...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div
        className="hidden lg:flex w-[42%] flex-col justify-between p-12 bg-foreground text-background select-none"
      >
        <span
        ...
        <div>
          ...
          >
            The art<br />of staying<br />focused.
          </h1>
          <p className="text-sm leading-relaxed opacity-35 max-w-[22ch]">
            A minimalist workspace for people who think in depth — not breadth.
          </p>
        </div>

        <p className="text-[11px] tracking-[0.1em] uppercase opacity-20">
          © 2026 Livo
        </p>
      </div>
      ...
      <div className="flex-1 relative flex items-center justify-center bg-background p-8">
        ...
        <div className="w-full max-w-sm animate-fade-up">
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">Good to have you back.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div>
              <Input placeholder="Password" type="password" autoComplete="current-password" {...register('password')} error={!!errors.password} />
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 bg-background text-xs text-muted-foreground/50">or continue with</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" onClick={() => handleOAuth('google')} className="gap-2 h-10">
              <GoogleIcon /> Google
            </Button>
            <Button variant="outline" onClick={() => handleOAuth('github')} className="gap-2 h-10">
              <GithubIcon /> GitHub
            </Button>
          </div>
          ...
          <p className="text-sm text-center mt-8 text-muted-foreground">
            No account?{' '}
            <Link href="/auth/register" className="text-foreground font-medium underline underline-offset-4 decoration-border hover:decoration-foreground transition-all">
              Create one
            </Link>
          </p>
        </div>
