...
import { Sparkles, Send, X, Bot, User, FileText, Loader2, ArrowUpRight } from 'lucide-react';
...
interface AskMyNotesPanelProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectNote: (noteId: string) => void;
}

export function AskMyNotesPanel({ isOpen, onClose, onSelectNote }: AskMyNotesPanelProps) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'msg-welcome',
      role: 'assistant',
      content: "Hello! I'm your livo RAG assistant. Ask me anything about your notes, architecture, ideas, or documents.",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg: ChatMessage = {
        id: `error-${Date.now()}`,
        role: 'assistant',
        content: `Error: ${err?.message || 'Unable to reach RAG assistant service.'}`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  return (
    <aside
      id="ask-my-notes-panel"
      aria-label="Ask My Notes AI Assistant"
    ...
    >
      ...
      <div className="px-4 py-3.5 border-b border-stone-200 bg-white flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-xs font-semibold text-stone-900">Ask My Notes</h2>
            <p className="text-[10px] text-stone-500">RAG Assistant with Source Citations</p>
          </div>
        </div>
        <button
          id="close-rag-panel"
        ...
        {loading && (
          <div className="flex items-start gap-3">
            ...
            <div className="bg-white border border-stone-200 rounded-2xl rounded-tl-xs px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-xs text-stone-500">Searching chunks & synthesizing answer...</span>
            </div>
          </div>
        )}
